;(function ( $, window, document, undefined ) {
	...
	var pluginName = 'ik_slider',
		defaults = {
			'instructions': 'Slider control. Use the right and left arrow keys to increase or decrease the slider value. Use home key to set to minimum value and end key to set to maximum value.',
			'minValue': 0,
			'maxValue': 100,
			'nowValue': 0,
			'step': 1
		...
		if( !plugin.textfield.is(':text') ) {
			...
			plugin.knob = $('<div/>')
				.attr({
                    ...
                    'aria-label': this.options.instructions // add description */
				})
				.addClass('ik_knob')
...
Plugin.prototype.onKeyDown = function (event) {
    ...
    switch (event.keyCode) {
        ...
        case ik_utils.keys = 39: //right arrow
        ...
        case ik_utils.keys = 35:  //end key
			value = plugin.options.maxValue;
            plugin.setValue(value);
            break;
       
        case ik_utils.keys = 37:  //left arrow
        ...
        case ik_utils.keys = 36:  //home key
			value = plugin.options.minValue;
            plugin.setValue(value);
            break;
